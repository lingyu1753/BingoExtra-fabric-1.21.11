...
public class BiomeSearchWorker implements WorldWorkerManager.IWorker {
	...
	public BiomeSearchWorker(ServerLevel level, Player player, ItemStack stack, Biome biome, BlockPos startPos, List<BlockPos> prevPos) {
		...
		sampleSpace = BiomeCompassConfig.sampleSpaceModifier * BiomeUtils.getBiomeSize(level);
		maxSamples = BiomeCompassConfig.maxSamples;
		maxRadius = BiomeCompassConfig.radiusModifier * BiomeUtils.getBiomeSize(level);
	...
	public void start() {
		if (!stack.isEmpty() && stack.getItem() == ModItems.BIOME_COMPASS) {
			if (maxRadius > 0 && sampleSpace > 0) {
				BingoExtra.LOGGER.info("BiomeSearchWorker " + id + ": Starting search: " + sampleSpace + " sample space, " + maxSamples + " max samples, " + maxRadius + " max radius, " + prevPos.size() + " previous locations");
	...
	private void finishSample() {
		...
		if (radius > 500 && radius / 500 > lastRadiusThreshold) {
			if (!stack.isEmpty() && stack.getItem() == ModItems.BIOME_COMPASS) {
				stack.set(ModDataComponents.SEARCH_RADIUS, roundRadius(radius, 500));
			}
	...
	private void succeed() {
		BingoExtra.LOGGER.info("BiomeSearchWorker " + id + ": Search succeeded: " + getRadius() + " radius, " + samples + " samples");
		if (!stack.isEmpty() && stack.getItem() == ModItems.BIOME_COMPASS) {
			((BiomeCompassItem) stack.getItem()).succeed(stack, biomeId, x, z, prevPos, samples, BiomeCompassConfig.displayCoordinates);
		} else {
			BingoExtra.LOGGER.error("BiomeSearchWorker " + id + ": Invalid compass after search");
		}
		finished = true;
	}

	private void fail() {
		BingoExtra.LOGGER.info("BiomeSearchWorker " + id + ": Search failed: " + getRadius() + " radius, " + samples + " samples");
		if (!stack.isEmpty() && stack.getItem() == ModItems.BIOME_COMPASS) {
			((BiomeCompassItem) stack.getItem()).fail(stack, roundRadius(getRadius(), 500), samples);
		} else {
			BingoExtra.LOGGER.error("Invalid compass after search");
		}
		finished = true;
	}

	public void stop() {
		BingoExtra.LOGGER.info("BiomeSearchWorker " + id + ": Search stopped: " + getRadius() + " radius, " + samples + " samples");
		finished = true;
	}

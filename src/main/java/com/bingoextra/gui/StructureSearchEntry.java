...
public class StructureSearchEntry extends ObjectSelectionList.Entry<StructureSearchEntry> {
	...
	private final StructureCompassScreen parentScreen;
	...
	public StructureSearchEntry(StructureSearchList structuresList, Identifier structureId, Player player) {
		...
		this.xpLevels = 0;
		if (BingoExtra.xpLevelsForAllowedStructures.containsKey(structureId)) {
			int levels = BingoExtra.xpLevelsForAllowedStructures.get(structureId);
			if (levels > 3) {
				levels = 3;
			}
	...
	public void renderContent(GuiGraphics guiGraphics, int mouseX, int mouseY, boolean isHovering, float partialTick) {
		...
		guiGraphics.drawString(mc.font, Component.translatable(("string.bingoextra.source")).append(Component.literal(": " + StructureUtils.getStructureSource(structureId))), getX() + 5, getY() + (getHeight() / 2) - ((mc.font.lineHeight + 2) * 1), infoColor);
		guiGraphics.drawString(mc.font, Component.translatable(("string.bingoextra.group")).append(Component.literal(": ")).append(Component.translatable(StructureUtils.getStructureName(BingoExtra.structureIdsToGroupIds.get(structureId)))), getX() + 5, getY() + (getHeight() / 2) + ((mc.font.lineHeight + 2) * 0), infoColor);
		guiGraphics.drawString(mc.font, Component.translatable(("string.bingoextra.dimension")).append(Component.literal(": " + StructureUtils.structureDimensionsToString(BingoExtra.dimensionsForAllowedStructures.get(structureId)))), getX() + 5, getY() + (getHeight() / 2) + ((mc.font.lineHeight + 2) * 1), infoColor);
	}
	...
	public boolean isEnabled() {
		return BingoExtra.infiniteXp || player.experienceLevel >= xpLevels;
	}
	...
	public Identifier getGroupId() {
		return BingoExtra.structureIdsToGroupIds.get(structureId);
	}

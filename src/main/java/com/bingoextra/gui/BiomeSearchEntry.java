...
import com.bingoextra.sorting.BiomeDimensionSorting;
import com.bingoextra.sorting.BiomeNameSorting;
import com.bingoextra.sorting.BiomeSourceSorting;
import com.bingoextra.sorting.BiomeTagsSorting;
...
public class BiomeSearchEntry extends ObjectSelectionList.Entry<BiomeSearchEntry> {
	...
	public BiomeSearchEntry(BiomeSearchList biomesList, Identifier biomeId, Player player) {
		...
		if (BingoExtra.xpLevelsForAllowedBiomes.containsKey(biomeId)) {
			int xpLevels = BingoExtra.xpLevelsForAllowedBiomes.get(biomeId);
			if (xpLevels > 3) {
				xpLevels = 3;
			}
	...
	public void renderContent(GuiGraphics guiGraphics, int mouseX, int mouseY, boolean isHovering, float partialTick) {
		...
		if (parentScreen.getSortingCategory() instanceof BiomeNameSorting || parentScreen.getSortingCategory() instanceof BiomeSourceSorting || parentScreen.getSortingCategory() instanceof BiomeTagsSorting || parentScreen.getSortingCategory() instanceof BiomeDimensionSorting) {
			title = I18n.get("string.bingoextra.dimension");
			value = BiomeUtils.dimensionIdsToString(BingoExtra.dimensionsForAllowedBiomes.get(biomeId));
		}
		...
		String tagsLine = I18n.get("string.bingoextra.tags") + ": " + tags;
		...
		guiGraphics.drawString(mc.font, Component.translatable("string.bingoextra.source").append(Component.literal(": " + BiomeUtils.getBiomeSource(parentScreen.level, biomeId))), getX() + 5, getY() + (getHeight() / 2) + ((mc.font.lineHeight + 2) * 1), infoColor);
	}
	...
	public boolean isEnabled() {
		// TODO: is it safe to trust the ClientPlayer's experience level or should it be included in a BiomeSyncPacket?

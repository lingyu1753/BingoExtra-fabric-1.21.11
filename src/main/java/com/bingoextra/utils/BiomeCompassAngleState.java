...
public class BiomeCompassAngleState extends NeedleDirectionHelper {

	private final NeedleDirectionHelper.Wobbler wobbler;
	private final RandomSource random = RandomSource.create();

	public BiomeCompassAngleState() {
		super(true);
	...
	protected float calculate(ItemStack stack, ClientLevel level, int seed, ItemOwner owner) {
		...
		if (stack.getItem() == ModItems.BIOME_COMPASS) {
			BiomeCompassItem compassItem = (BiomeCompassItem) stack.getItem();
			if (compassItem.getCompassState(stack) == BiomeCompassState.FOUND) {
				pos = new GlobalPos(level.dimension(), new BlockPos(stack.getOrDefault(ModDataComponents.FOUND_X, 0), 0, stack.getOrDefault(ModDataComponents.FOUND_Z, 0)));
			}
		}

...
public class StructureUtils {
	...
	public static Identifier getGroupForStructure(ServerLevel level, Structure structure) {
		...
		return Identifier.fromNamespaceAndPath(BingoExtra.MOD_ID, "none");
	}
	...
	public static boolean structureIsBlacklisted(ServerLevel level, Structure structure) {
		final List<String> structureBlacklist = StructureCompassConfig.structureBlacklist;
	...
	public static int getXpLevelsForStructure(ServerLevel serverLevel, Identifier structureKey) {
		int xpLevels = StructureCompassConfig.defaultXpLevel;
		for (String structureRegex : StructureCompassConfig.perStructureXpLevels.keySet()) {
			if (structureKey.toString().matches(convertToRegex(structureRegex))) {
				xpLevels = StructureCompassConfig.perStructureXpLevels.get(structureRegex);
				if (xpLevels > 3) {
					xpLevels = 3;
				}
				break;
	...
	public static String getStructureName(Identifier id) {
		...
		if (StructureCompassConfig.translateStructureNames) {
			name = I18n.get(Util.makeDescriptionId("structure", id));
		}
		if (name.equals(Util.makeDescriptionId("structure", id)) || !StructureCompassConfig.translateStructureNames) {
			name = id.toString();
			if (name.contains(":")) {

...
public class StructureCompassAngleState extends NeedleDirectionHelper {
	...
	public StructureCompassAngleState() {
		super(true);
	...
	protected float calculate(ItemStack stack, ClientLevel level, int seed, ItemOwner owner) {
		...
		if (stack.getItem() == ModItems.STRUCTURE_COMPASS) {
			StructureCompassItem compassItem = (StructureCompassItem) stack.getItem();
			if (compassItem.getCompassState(stack) == StructureCompassState.FOUND) {
